...
mm_9t_null <- lmer(value ~ (1 | name), data = df)
summary(mm_t)
...
```{r}
fit.lqmm <- lqmm(fixed = value ~ group, random = ~1, group = name, tau = c(0.25,0.5, 0.75), nK = 7, type = "normal", data = df, control = c(LP_max_iter = 5000))
summary(fit.lqmm, R = 500, seed = 2)
```

```{r}
fit.log.lqmm <- lqmm(fixed = log(value) ~ group, random = ~1, group = name, tau = c(0.25,0.5, 0.75), nK = 7, type = "normal", data = df, control = c(LP_max_iter = 5000))
summary(fit.log.lqmm, R = 500, seed = 2)
```


# Hack together additional tests for differences in distributions
```{r}
...
Make dabest plots for the above comparisons. Use dabest_plot.
```{r}
